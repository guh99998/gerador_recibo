...
run_corpo.bold = True
corpo.add_run(
    f'", inscrito no CPF n° {formatar_doc(cpf_cliente)}, a importância de R${valor_recibo:.2f} ({number_to_long_number(valor_recibo)}), referente ao {referencia_recibo}.'
)

data = document.add_paragraph()
data.alignment = WD_ALIGN_PARAGRAPH.CENTER
data_run = data.add_run(
    f"{cidade_recibo} - {estado_recibo.upper()}, {formatar_data(data_recibo)}."
)
...
documento_recebedor_run = documento_recebedor.add_run(
    f"Documento: {formatar_doc(doc_recebedor)}"
)
